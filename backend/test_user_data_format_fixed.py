...
def test_user_data_format():
    ...
    try:
        ...
        # Create a properly structured TestFeedback
        feedback_obj = TestFeedback(
            strengths=test_data.get('strengths', []),
            improvements=test_data.get('improvements', []),
            detailed_feedback={
                'fluency_coherence': test_data['feedback']['fluency'],
                'lexical_resource': test_data['feedback']['vocabulary'],
                'grammatical_accuracy': test_data['feedback']['grammar'],
                'pronunciation': test_data['feedback']['pronunciation']
            }
        )
        ...
        print(f"   📊 Test result feedback strengths: {len(test_result.feedback.strengths)}")
        print(f"   📊 Test result feedback improvements: {len(test_result.feedback.improvements)}")
...
def test_actual_save_function():
    """Test the actual save function with user's data format."""
    
    print("\n🧪 Testing Actual Save Function")
    print("=" * 50)
    
    # User's data format (single test result, not array)
    test_data = {
        "answers": {
            "Part 1": {
    ...
    try:
        ...
        email = "test_user_format@example.com"
        result = service.save_test_result(email, test_data)
...
if __name__ == "__main__":
    ...
    if success:
        ...
        # Test actual save function
        save_success = test_actual_save_function()
        if save_success:
            print("✅ Actual save function works with your data format!")
        else:
            print("❌ Save function needs adjustment")
    else:
